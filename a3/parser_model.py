...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        w1, b1 = torch.empty(n_features * self.embed_size, hidden_size), torch.empty(hidden_size)
        nn.init.xavier_uniform_(w1)
        nn.init.uniform_(b1)
        self.embed_to_hidden_weight, self.embed_to_hidden_bias = nn.Parameter(w1), nn.Parameter(b1)

        self.dropout = nn.Dropout(p=dropout_prob)

        w2, b2 = torch.empty(hidden_size, n_classes), torch.empty(n_classes)
        nn.init.xavier_uniform_(w2)
        nn.init.uniform_(b2)
        self.hidden_to_logits_weight, self.hidden_to_logits_bias = nn.Parameter(w2), nn.Parameter(b2)
    ...
    def embedding_lookup(self, w):
        ...
        x = self.embeddings[w.flatten()].view(-1, self.n_features * self.embed_size)
    ...
    def forward(self, w):
        ...
        h = nn.functional.relu(torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias[None])
        logits = torch.matmul(h, self.hidden_to_logits_weight) + self.hidden_to_logits_bias[None]

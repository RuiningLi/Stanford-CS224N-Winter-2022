...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = -outsideVectors[outsideWordIdx] + np.dot(outsideVectors.T, yHat)
    gradOutsideVecs = np.outer(yHat, centerWordVec)
...
def negSamplingLossAndGradient(
    ...
    prob = sigmoid(np.dot(outsideVectors[indices], centerWordVec))
    prob[1:] = 1 - prob[1:]
    loss = -np.log(prob).sum()
    gradCenterVec = -(1 - prob[0]) * outsideVectors[outsideWordIdx] \
        + np.dot(outsideVectors[negSampleWordIndices].T, 1 - prob[1:])
    gradOutsideVecs = np.zeros_like(outsideVectors)
    uniqueWordIndices, idx, counts = np.unique(
        negSampleWordIndices, return_index=True, return_counts=True)
    gradOutsideVecs[uniqueWordIndices] += (np.outer(centerWordVec, 1 - prob[1 + idx]) * counts).T
    gradOutsideVecs[outsideWordIdx] -= (1 - prob[0]) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    gradOutsideVectors = np.zeros_like(outsideVectors)

    ### YOUR CODE HERE (~8 Lines)

    currentCenterWordInd = word2Ind[currentCenterWord]
    currentCenterWordVec = centerWordVectors[currentCenterWordInd]
    for w in outsideWords:
        currentOutsideWordIdx = word2Ind[w]
        currentLoss, currentGradCenterVec, currentGradOutsideVecs = word2vecLossAndGradient(
            currentCenterWordVec,
            currentOutsideWordIdx,
            outsideVectors,
            dataset
        )
        loss += currentLoss
        gradCenterVecs[currentCenterWordInd] += currentGradCenterVec

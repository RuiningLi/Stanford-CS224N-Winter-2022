...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = pack_padded_sequence(self.model_embeddings.source(source_padded), lengths=source_lengths)
        output, (h_n, c_n) = self.encoder(X)
        enc_hiddens = pad_packed_sequence(output)[0].permute(1,0,2)
        dec_init_state = (
            self.h_projection(h_n.permute(1,0,2).reshape(-1, 2*self.hidden_size)),
            self.c_projection(c_n.permute(1,0,2).reshape(-1, 2*self.hidden_size)))
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t = torch.bmm(dec_hidden.unsqueeze(dim=1), enc_hiddens_proj.permute(0,2,1)).squeeze()
        ...
        a_t = torch.bmm(alpha_t.unsqueeze(dim=1), enc_hiddens).squeeze()
